...
def generate_sales_orders_stg_sql():
    """
    Generate SQL for creating the sales_orders_comp_stg intermediate table
    based on the provided mapping document.
...
def generate_sales_orders_sql():
    """
    Generate SQL for creating the sales_orders_comp main table
    based on the provided mapping document.
    ...
    LEFT JOIN 
        (SELECT DISTINCT 
         ...
         UNION
         SELECT DISTINCT 
             refth AS referenceto, refnh AS referencefrom, drqhx AS requestno, 
             seqnh AS seqnum, extth AS exttax, seqni AS seqline, whcdi AS whcode 
    ...
    LEFT JOIN 
        b_source.ord_addr g
    ON TRIM(a.ShipToNumber) = TRIM(CONCAT(g.CustNo, g.CustName))
    LEFT JOIN 
...
def main():
    """
    Main function to generate and execute SQL queries
    """
