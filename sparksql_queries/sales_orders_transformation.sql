-- Step 1: Create Intermediate table with name s_shared.sales_orders_hdr_itm_lots
%sql
...
LEFT JOIN
    b_lots.plool b
ON
    TRIM(a.refto) = TRIM(b.refto) AND
    TRIM(a.refno) = TRIM(b.refno) AND
    DATA_OPERATION <> 'D';
...
SELECT
    ...
    b.OrderQuantityOriginal,
    b.OrderQuantityBase,
    b.CancelledQuantityOriginal,
    NULL AS CancelledQuantityBase,
    b.DeliveredQuantityOriginal,
    b.OpenQuantityOrginal,
    b.OpenQuantityBase,
...
FROM
    s_shared.sales_orders_hdr_itm_lots b
LEFT JOIN
    (SELECT * FROM b_lots.plooh WHERE DATA_OPERATION <> 'D') a
    ON TRIM(b.OrderNumber) = TRIM(CONCAT(a.refto, a.refno))
LEFT JOIN
    ...
    ON 1=1
LEFT JOIN
    (SELECT DISTINCT refto, refno, dreql, seqnl, exttx, seqne, whcda
     FROM b_lots.ploal
     WHERE DATA_OPERATION <> 'D' AND TRIM(whcda) != '' AND whcda IS NOT NULL
     ...
     SELECT DISTINCT refth AS refto, refnh AS refno, drqhx AS dreql, seqnh AS seqnl, 
            extth AS exttx, seqni AS seqne, whcdi AS whcda
     FROM b_lots.phihm
     WHERE DATA_OPERATION <> 'D' AND TRIM(whcdi) != '' AND whcdi IS NOT NULL) e
    ON TRIM(b.OrderNumber) = TRIM(CONCAT(e.refto, e.refno))
    AND b.dreqlc = e.dreql
    AND b.LineNumber = e.seqnl
    AND TRIM(b.exttx) = TRIM(e.exttx)
    AND b.seqne = e.seqne
LEFT JOIN
    (SELECT CUSNX, CUSAX, PRODX, MAX(SCTCX) AS SCTCX
     FROM b_lots.plcam
     WHERE DATA_OPERATION <> 'D'
     GROUP BY CUSNX, CUSAX, PRODX) PLCAM
    ON TRIM(CONCAT(PLCAM.CUSNX, PLCAM.CUSAX)) = TRIM(b.ShipToNumber)
    AND TRIM(PLCAM.PRODX) = TRIM(b.MaterialNumber)
LEFT JOIN
    lots_staging.PLPMS_stg PLPMS
    ON TRIM(PLPMS.PRODX) = TRIM(b.MaterialNumber)
    AND DATA_OPERATION <> 'D'
LEFT JOIN
    b_lots.plcbe g
    ON TRIM(b.ShipToNumber) = TRIM(CONCAT(g.cusnx, g.cusax))
LEFT JOIN
    (SELECT cusnx, cusax, cusnx4
     FROM b_lots.plcbe
     WHERE DATA_OPERATION <> 'D') h
    ON TRIM(b.SoldToNumber) = TRIM(CONCAT(h.cusnx, h.cusax))
LEFT JOIN
...
LEFT JOIN
    (SELECT a.*
     ...
     JOIN (SELECT MAX(from_date) latest_date
...
LEFT JOIN
    (SELECT MAX(dlshlc) AS dlshlc, refno, refto, dreql, seqnl
     FROM b_lots.lecotw1
     GROUP BY refno, refto, seqnl, dreql) i
    ON TRIM(b.OrderNumber) = TRIM(CONCAT(i.refno, i.refto))
    AND b.dreqlc = i.dreql
    AND b.LineNumber = i.seqnl
LEFT JOIN
    b_user_managed.rtbl_lots_ref_type_category_bo_scope j
    ON TRIM(a.refto) = TRIM(j.ref_type)
LEFT JOIN
    (SELECT availability_descr, prodx
...
LEFT JOIN
    ...
    ON TRIM(b.PayerNumber) = TRIM(CONCAT(m.cusnx, m.cusax))
WHERE
    DATA_OPERATION <> 'D'
    AND YEAR(CAST(TO_DATE(FROM_UNIXTIME(UNIX_TIMESTAMP(CAST(a.datoxc AS STRING), 'yyyyMMdd'))) AS DATE)) >= YEAR(CURRENT_DATE()) - 3;