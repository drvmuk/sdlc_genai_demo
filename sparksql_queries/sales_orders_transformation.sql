...
WITH registry_data AS (
    ...
    AND TRIM(sourceSystemValue) = 'LOTS'
),
plcam_data AS (
    SELECT 
        CUSNX, 
...
phihm_data AS (
    ...
    FROM b_lots.phihm 
    WHERE DATA_OPERATION <> 'D'
    GROUP BY refth, refnh, seqnh, extth, seqni
),
lecotw1_data AS (
    SELECT 
        MAX(dlshlc) AS dlshlc, 
        refno, 
        refto, 
        dreql, 
        seqnl
    FROM b_lots.lecotw1 
    GROUP BY refno, refto, seqnl, dreql
),
warehouse_data AS (
    SELECT DISTINCT 
    ...
    SELECT DISTINCT 
        ...
        whcdi AS whcda 
    FROM b_lots.phihm 
    WHERE DATA_OPERATION <> 'D' AND TRIM(whcdi) != '' AND whcdi IS NOT NULL
),
latest_currency_conversion AS (
    SELECT a.* 
    FROM s_shared.currency_conversion a
    JOIN (
        SELECT MAX(from_date) AS latest_date 
...
LEFT JOIN 
    b_lots.plool b ON TRIM(a.OrderNumber) = TRIM(CONCAT(b.refto, b.refno)) AND a.LineNumber = b.seqnl
LEFT JOIN 
    registry_data registry
LEFT JOIN 
    warehouse_data e ON TRIM(a.OrderNumber) = TRIM(CONCAT(e.refto, e.refno)) 
                    AND a.dreqlc = e.dreql 
                    AND a.LineNumber = e.seqnl 
                    AND TRIM(a.exttx) = TRIM(e.exttx) 
                    AND a.seqne = e.seqne
LEFT JOIN 
    b_lots.plcbe g ON TRIM(a.ShipToNumber) = TRIM(CONCAT(g.cusnx, g.cusax)) AND g.DATA_OPERATION <> 'D'
LEFT JOIN 
...
LEFT JOIN 
    plcam_data PLCAM ON TRIM(CONCAT(PLCAM.CUSNX, PLCAM.CUSAX)) = TRIM(a.ShipToNumber) 
                     AND TRIM(PLCAM.PRODX) = TRIM(a.MaterialNumber)
LEFT JOIN 
    lots_staging.PLPMS_stg PLPMS ON TRIM(PLPMS.PRODX) = TRIM(a.MaterialNumber) 
                                AND PLPMS.DATA_OPERATION <> 'D'
LEFT JOIN 
    phihm_data d ON TRIM(a.OrderNumber) = TRIM(CONCAT(d.refth, d.refnh)) 
                AND a.LineNumber = d.seqnh 
                AND TRIM(a.exttx) = TRIM(d.extth) 
                AND a.seqne = d.seqni
LEFT JOIN 
    latest_currency_conversion cur ON TRIM(a.CurrencyType) = TRIM(cur.FromCurrency)
LEFT JOIN 
    lecotw1_data i ON TRIM(a.OrderNumber) = TRIM(CONCAT(i.refno, i.refto)) 
                  AND a.dreqlc = i.dreql 
                  AND a.LineNumber = i.seqnl
LEFT JOIN 
    b_user_managed.rtbl_lots_ref_type_category_bo_scope j ON TRIM(a.refto) = TRIM(j.ref_type)
LEFT JOIN 
    (SELECT availability_descr, prodx FROM b_lots.lmpmt03) k ON TRIM(a.MaterialNumber) = TRIM(k.prodx)
LEFT JOIN 
    b_user_managed.rtbl_lots_market_region_cco l ON TRIM(a.entch) = TRIM(l.entch)
WHERE 
    a.DATA_OPERATION <> 'D'
    AND YEAR(CAST(TO_DATE(FROM_UNIXTIME(UNIX_TIMESTAMP(CAST(a.CreateDate AS STRING), 'yyyy-MM-dd'))) AS DATE)) >= YEAR(CURRENT_DATE()) - 3;