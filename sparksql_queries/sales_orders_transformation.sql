...
LEFT JOIN 
  ...
  ON TRIM(a.PayerNumber) = TRIM(CONCAT(payer.cusnx, payer.cusax))
LEFT JOIN 
  (
    SELECT 
...
LEFT JOIN 
  (
    ...
    JOIN 
      (
        SELECT 
          MAX(from_date) latest_date 
        FROM 
          s_shared.currency_conversion
      ) b
...
LEFT JOIN 
  ...
  ON TRIM(a.OrderNumber) = TRIM(CONCAT(i.refno, i.refto))
