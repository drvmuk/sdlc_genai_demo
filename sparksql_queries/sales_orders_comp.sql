%sql
-- Create Main table "sales_orders_comp"
...
LEFT JOIN 
    b_source.ord_addr g
ON 
    TRIM(a.ShipToNumber) = TRIM(CONCAT(g.CustNo, g.CustName))
    AND g.delete_flag <> 'D'
LEFT JOIN 
...
LEFT JOIN 
    (
        SELECT 
            CustNo, CustName, PRODX, MAX(SCTCX) AS SCTCX 
        FROM 
            b_source.ord_channel 
        WHERE 
            delete_flag <> 'D' 
        GROUP BY 
            CustNo, CustName, PRODX
    ) PLCAM
ON 
...
LEFT JOIN 
    (
        SELECT 
            MAX(dlshlc) AS dlshlc, referencefrom, referenceto, requestno, seqnum
        FROM 
            b_source.ord_type 
        GROUP BY 
            referencefrom, referenceto, seqnum, requestno
    ) i
ON 
...
ON 
    TRIM(a.referenceto) = TRIM(j.ref_type)
LEFT JOIN 
...
LEFT JOIN 
    (
        ...
        FROM 
            s_master.currency a
        JOIN 
            (SELECT MAX(from_date) latest_date FROM s_master.currency) b
        ON 
            TRIM(a.from_date) = TRIM(b.latest_date)
    ) cur
ON 
