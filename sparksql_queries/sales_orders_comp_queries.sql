    ...
    (SELECT 
     ...
     FROM 
        (SELECT 
         ...
         WHERE delete_flag <> 'D' AND TRIM(whcdi) != '' AND whcdi IS NOT NULL
        ) site_data
    ) e ON TRIM(b.OrderNumber) = TRIM(CONCAT(e.referenceto, e.referencefrom))
...
LEFT JOIN 
    (SELECT CustNo, CustName, PRODX, MAX(SCTCX) AS SCTCX FROM b_source.ord_channel WHERE delete_flag <> 'D' GROUP BY CustNo, CustName, PRODX) PLCAM 
    ON TRIM(CONCAT(PLCAM.CustNo, PLCAM.CustName)) = TRIM(a.ShipToNumber) AND TRIM(PLCAM.PRODX) = TRIM(b.MaterialNumber)
LEFT JOIN 
...
LEFT JOIN 
    ...
    ) cur ON TRIM(a.CurrencyType) = TRIM(cur.FromCurrency)
LEFT JOIN 
    (SELECT 
        refth, 
        refnh, 
    ...
    ) d ON TRIM(b.OrderNumber) = TRIM(CONCAT(d.refth, d.refnh))
...
LEFT JOIN 
    (SELECT 
     ...
     GROUP BY referencefrom, referenceto, seqnum, requestno
    ) i ON TRIM(a.OrderNumber) = TRIM(CONCAT(i.referencefrom, i.referenceto))
