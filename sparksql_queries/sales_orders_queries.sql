...
LEFT JOIN 
    (SELECT 
     ...
     FROM 
        b_source.ord_site 
     WHERE 
        delete_flag <> 'D' AND trim(whcode) != '' AND whcode IS NOT NULL
     UNION
     SELECT 
        referenceto, referencefrom, requestno, seqnum, exttax, seqline, whcode 
     FROM 
     ...
     FROM 
        b_source.ord_reg 
     WHERE 
        delete_flag <> 'D' AND trim(whcdi) != '' AND whcdi IS NOT NULL) e
ON 
    trim(a.OrderNumber) = trim(concat(e.referenceto, e.referencefrom))
    AND a.requestnoc = e.requestno
    AND a.LineNumber = e.seqnum
    AND trim(a.exttax) = trim(e.exttax)
    AND a.seqline = e.seqline
LEFT JOIN 
    b_source.ord_addr g
ON 
    trim(a.ShipToNumber) = trim(CONCAT(g.CustNo, g.CustName)) AND g.delete_flag <> 'D'
LEFT JOIN 
    (SELECT CustNo, CustName, CustNo4 FROM b_source.ord_addr WHERE delete_flag <> 'D') h
ON 
    trim(a.SoldToNumber) = trim(CONCAT(h.CustNo, h.CustName))
LEFT JOIN 
    (SELECT CustNo, CustName, CustNo4 FROM b_source.ord_addr WHERE delete_flag <> 'D') m
ON 
    trim(a.PayerNumber) = trim(CONCAT(m.CustNo, m.CustName))
LEFT JOIN 
...
ON 
    trim(CONCAT(PLCAM.CustNo, PLCAM.CustName)) = trim(a.ShipToNumber)
    AND trim(PLCAM.PRODX) = trim(b.MaterialNumber)
LEFT JOIN 
    b_source.ord_stat PLPMS
ON 
    trim(PLPMS.PRODX) = trim(b.MaterialNumber) AND PLPMS.delete_flag <> 'D'
LEFT JOIN 
    (SELECT a.* 
...
ON 
    trim(a.OrderNumber) = trim(concat(d.refth, d.refnh))
    AND a.LineNumber = d.seqnh
    AND trim(a.exttax) = trim(d.extth)
    AND a.seqline = d.seqni
LEFT JOIN 
...
ON 
    trim(a.OrderNumber) = trim(concat(i.referencefrom, i.referenceto))
    AND a.requestnoc = i.requestno
    AND a.LineNumber = i.seqnum
LEFT JOIN 
    b_source.Boscope j
ON 
    trim(a.referenceto) = trim(j.ref_type) AND j.delete_flag <> 'D'
LEFT JOIN 
